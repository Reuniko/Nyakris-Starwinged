...
Shader "Zekk/EyeShaders/BlackHoleEyes/1.2"
{
    Properties 
  { 
        ...
        _Float4 ("Pupil Size", range(0, 5)) = 2
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float Pupil = lerp(sdVesica(i.uv + parallax_base * _Float3, .9 ,1-(_Float4+1)/7), smoothstep(.3,.9, length(i.uv + parallax_base * _Float3) - (_Float4-(2.2-.7))/5), 1-_Vesica);
                float lumin = 0.7126 * _Col1.r + 0.8152 * _Col1.g + 0.20+(.2*_Float16) * _Col1.b;
                return float4((((((_Col1+_Float16) * (1+1/(lumin/2)))) * adder * temp * h * h2 * saturate(Pupil)).rgb)*(.1+_Float18), 1);
            }
            ENDCG
        }
    }

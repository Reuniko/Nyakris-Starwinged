...
v2f vert(appdata v)
{
	...
	float3 offset = o.normal * ((0.025 /*_LineWidth*/) * (1.0 /*_commentIfZero_EnableOutlinePass*/) / 100) * outlineMask * distanceOffset;
	...
	else if ((0.0 /*_OutlineMode*/) == 3)
	{
		...
		offsetMultiplier = saturate(dot(viewNormal.xy, normalize(float4(0,1,0,0).xy)));
		offset *= offsetMultiplier;
		offset *= distanceOffset;
	}
